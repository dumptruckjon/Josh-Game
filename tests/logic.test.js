...
// ---------- 🩺 Boo-Boo Clinic: day math, save validation, round picker ----------

test("dayIndex: same local day → same index; midnight rollover → exactly +1 (DST-safe)", () => {
  // Two times on the same local calendar day.
  assert.equal(
    L.dayIndex(new Date(2026, 6, 14, 0, 1)),
    L.dayIndex(new Date(2026, 6, 14, 23, 58)),
    "same local day must map to the same index"
  );
  // 11:58pm → 00:02 the next day = exactly one day apart.
  assert.equal(
    L.dayIndex(new Date(2026, 6, 15, 0, 2)) - L.dayIndex(new Date(2026, 6, 14, 23, 58)),
    1, "midnight rollover must be exactly +1"
  );
  // Across the US spring-forward DST boundary (Mar 8, 2026): still exactly +1.
  assert.equal(
    L.dayIndex(new Date(2026, 2, 9, 12, 0)) - L.dayIndex(new Date(2026, 2, 8, 12, 0)),
    1, "a DST-shortened day must still count as one day"
  );
  // Across the fall-back boundary (Nov 1, 2026): still exactly +1.
  assert.equal(
    L.dayIndex(new Date(2026, 10, 2, 12, 0)) - L.dayIndex(new Date(2026, 10, 1, 12, 0)),
    1, "a DST-lengthened day must still count as one day"
  );
  // Year wrap: Dec 31 → Jan 1 = +1.
  assert.equal(
    L.dayIndex(new Date(2027, 0, 1, 8, 0)) - L.dayIndex(new Date(2026, 11, 31, 8, 0)),
    1, "the year wrap must be exactly +1"
  );
});

test("eggHatches: never the same day, always a later day, never a clock jumped backwards", () => {
  const today = L.dayIndex(new Date(2026, 6, 14, 12, 0));
  assert.equal(L.eggHatches(today, today), false, "an egg never hatches the day it was laid");
  assert.equal(L.eggHatches(today, today + 1), true, "the egg hatches on the NEXT day");
  assert.equal(L.eggHatches(today, today + 30), true, "…or any later day (absence costs nothing)");
  assert.equal(L.eggHatches(today, today - 1), false, "a clock jumped backwards keeps the egg cosy");
  assert.equal(L.eggHatches(NaN, today), false, "garbage laid-day never hatches");
  assert.equal(L.eggHatches(today, NaN), false, "garbage today never hatches");
  assert.equal(L.eggHatches(undefined, today), false, "missing laid-day never hatches");
});

test("validateClinicStore: the migration guard — garbage in, fresh world out (never a throw)", () => {
  const V = L.validateClinicStore;
  // Anything structurally wrong → null (the caller starts a fresh world).
  for (const bad of [
    null, undefined, 42, "junk", [], {},
    { v: 2, healed: [] },                                  // future version
    { v: 1 },                                              // healed missing
    { v: 1, healed: {} },                                  // healed not an array
    { v: 1, healed: [{ name: "Bunny", sticker: "⭐" }] },  // record missing sp
    { v: 1, healed: [{ sp: "🐰", name: "Bunny" }] },       // record missing sticker
    { v: 1, healed: [{ sp: "🐰", name: "Bunny", sticker: "⭐", fed: [] }] }, // fed not an object
    { v: 1, healed: [{ sp: "🐰", name: "Bunny", sticker: "⭐", fed: { "🥕": -1 } }] }, // negative count
    { v: 1, healed: [], egg: { laidOnDay: "tomorrow" } },  // egg day not a number
    { v: 1, healed: [], hatched: [{}] },                   // hatchling missing sp
  ]) {
    assert.equal(V(bad), null, `must reject: ${JSON.stringify(bad)}`);
  }
  // A valid world round-trips, keeping the pup collar / hero color field.
  const world = {
    v: 1,
    healed: [
      { sp: "🐰", name: "Bunny", kind: "emoji", sticker: "⭐", fed: { "🥕": 3 } },
      { sp: "pup:Chase", name: "Chase", kind: "pup", sticker: "❤️", fed: {}, c: "#2b6cff" },
    ],
    egg: { laidOnDay: 20648 },
    hatched: [{ sp: "🐤", name: "Chick" }],
  };
  assert.deepEqual(V(world), world, "a valid world must round-trip unchanged");
  // Unknown extra fields are dropped, missing optionals get safe defaults.
  const messy = V({
    v: 1, junk: true,
    healed: [{ sp: "🐰", name: "Bunny", sticker: "⭐", surprise: 1 }],
  });
  assert.deepEqual(messy, { v: 1, healed: [{ sp: "🐰", name: "Bunny", kind: "emoji", sticker: "⭐", fed: {} }], egg: null, hatched: [] });
});

test("makeClinicRound: rotation without photocopies, honoring per-patient ailments", () => {
  const rng = mulberry32(77);
  const patients = [
    { sp: "🐰", name: "Bunny", kind: "emoji", classes: null },
    { sp: "🐱", name: "Cat", kind: "emoji", classes: null },
    { sp: "pup:Chase", name: "Chase", kind: "pup", classes: ["ouchie", "dirty", "tangled"] },
    { sp: "truck", name: "Little Dump Truck", kind: "truck", classes: ["dirty", "tangled", "rumbly"] },
  ];
  const classes = content.CLINIC.AILMENTS.map((a) => a.id);
  let lastPatient = null, lastClass = null;
  for (let i = 0; i < 800; i++) {
    const wanted = 1 + (i % 3); // exercise 1-, 2- and 3-ailment rounds
    const r = L.makeClinicRound({ patients, classes, lastPatient, lastClass, ailments: wanted, maxCare: 6 }, rng);
    assert.notEqual(r.patient.sp, lastPatient, "never the same patient back-to-back");
    assert.notEqual(r.ailments[0], lastClass, "never the same opening boo-boo back-to-back");
    const allowed = r.patient.classes || classes;
    for (const a of r.ailments) assert.ok(allowed.includes(a), `${r.patient.name} can't have ${a}`);
    assert.equal(new Set(r.ailments).size, r.ailments.length, "a patient's boo-boos are distinct");
    assert.ok(r.ailments.length >= 1 && r.ailments.length <= Math.min(wanted, allowed.length), "ailment count within bounds");
    assert.ok(r.careTaps >= 4 && r.careTaps <= 6, `care taps in [4,6], got ${r.careTaps}`);
    assert.ok(r.variant === 0 || r.variant === 1, "variant is 0 or 1");
    lastPatient = r.patient.sp;
    lastClass = r.ailments[0];
  }
});

test("makeClinicRound: every ailment class a patient allows is actually reachable", () => {
  const rng = mulberry32(99);
  const patients = [{ sp: "🐰", name: "Bunny", kind: "emoji", classes: null }, { sp: "🐱", name: "Cat", kind: "emoji", classes: null }];
  const classes = content.CLINIC.AILMENTS.map((a) => a.id);
  const seen = new Set();
  let lastPatient = null, lastClass = null;
  for (let i = 0; i < 400; i++) {
    const r = L.makeClinicRound({ patients, classes, lastPatient, lastClass, ailments: 1 }, rng);
    seen.add(r.ailments[0]);
    lastPatient = r.patient.sp; lastClass = r.ailments[0];
  }
  assert.equal(seen.size, classes.length, "rotation must eventually visit every boo-boo class");
});
