...
// ---------- 🩺 Boo-Boo Clinic: the care truth tables ----------
test("clinic: every boo-boo has EXACTLY one remedy tool (and the feather heals nothing)", () => {
  const CL = content.CLINIC;
  assert.ok(CL && Array.isArray(CL.TOOLS) && Array.isArray(CL.AILMENTS), "CLINIC content must exist");
  for (const ail of CL.AILMENTS) {
    const remedies = CL.TOOLS.filter((t) => t.heals === ail.id);
    assert.equal(remedies.length, 1, `boo-boo "${ail.id}" needs exactly one remedy, got ${remedies.length}`);
  }
  // Every tool's heals target is a real ailment class (or null = pure comedy).
  const classIds = new Set(CL.AILMENTS.map((a) => a.id));
  for (const t of CL.TOOLS) {
    assert.ok(t.heals === null || classIds.has(t.heals), `tool "${t.id}" heals unknown class "${t.heals}"`);
  }
  const feather = CL.TOOLS.find((t) => t.id === "feather");
  assert.ok(feather && feather.heals === null, "the tickle feather must heal NOTHING (it is always pure comedy)");
  // Tool ids and emoji are unique (a tray with twins would be confusing).
  assert.equal(new Set(CL.TOOLS.map((t) => t.id)).size, CL.TOOLS.length, "tool ids must be unique");
  assert.equal(new Set(CL.TOOLS.map((t) => t.emoji)).size, CL.TOOLS.length, "tool emoji must be unique");
});

test("clinic: every tool carries its full voice + icon kit (gag AND care)", () => {
  for (const t of content.CLINIC.TOOLS) {
    for (const field of ["emoji", "name", "gagIcon", "gagSay", "careIcon", "careSay"]) {
      assert.ok(typeof t[field] === "string" && t[field].length > 0, `tool "${t.id}" needs ${field}`);
    }
  }
  for (const a of content.CLINIC.AILMENTS) {
    assert.ok(typeof a.say === "string" && a.say.length > 0, `boo-boo "${a.id}" needs a spoken name`);
    assert.ok(typeof a.icon === "string" && a.icon.length > 0, `boo-boo "${a.id}" needs an icon`);
  }
});

test("clinic: every animal patient names itself (it must exist in ANIMALS)", () => {
  const byEmoji = new Map(content.ANIMALS.map((a) => [a.emoji, a.name]));
  assert.ok(content.CLINIC.PATIENT_ANIMALS.length >= 8, "the clinic needs a rich animal roster");
  for (const sp of content.CLINIC.PATIENT_ANIMALS) {
    assert.ok(byEmoji.has(sp), `clinic patient ${sp} must exist in ANIMALS so its picture names itself`);
  }
  assert.equal(new Set(content.CLINIC.PATIENT_ANIMALS).size, content.CLINIC.PATIENT_ANIMALS.length, "no duplicate patients");
});

test("clinic: treats, stickers and hatchlings are distinct and speakable", () => {
  const CL = content.CLINIC;
  assert.ok(CL.TREATS.length >= 4, "the meadow treat tray needs variety");
  assert.equal(new Set(CL.TREATS.map((t) => t.emoji)).size, CL.TREATS.length, "treat emoji must be unique");
  for (const t of CL.TREATS) {
    assert.ok(typeof t.name === "string" && t.name.length > 0, `treat ${t.emoji} needs a spoken name`);
    assert.ok(typeof t.say === "string" && t.say.length > 0, `treat ${t.emoji} needs a happy reaction line`);
  }
  assert.ok(CL.STICKERS.length >= 3, "the sticker tray offers a real choice");
  assert.equal(new Set(CL.STICKERS).size, CL.STICKERS.length, "stickers must be distinct");
  assert.ok(CL.HATCHLINGS.length >= 2, "the egg needs more than one possible surprise");
  assert.equal(new Set(CL.HATCHLINGS.map((h) => h.emoji)).size, CL.HATCHLINGS.length, "hatchling emoji must be unique");
  for (const h of CL.HATCHLINGS) {
    assert.ok(typeof h.name === "string" && h.name.length > 0, `hatchling ${h.emoji} needs a name`);
  }
});
